...
| resource-compose | Gradle plugin 已对齐 Compose 1.7.3；runtime 因 OHOS 仍保留 1.6.1-KBA-001 | Gradle plugin 已对齐 Compose 1.7.3；runtime 因 OHOS 仍保留 1.6.1-KBA-001 |
...
`resource-compose` 不是 MR 生成和 `resource-core` 运行时的必需依赖。已经有自定义 Compose wrapper 的工程可以只依赖
`resource-core`。bytemain 源码已把 Compose Gradle plugin 对齐到 1.7.3，给后续接入保留；但
`resource-compose` 的 `org.jetbrains.compose.runtime/foundation` 仍保留在 `1.6.1-KBA-001`，因为纯 JetBrains
`1.7.3` artifact 没有 `ohos_arm64` native variant。默认 GitHub Packages workflow 仍只发布
`resource-core` 和 generator plugin。只有明确要为新版本发布 Compose wrapper artifact 时，才打开
